...
def main():
    ...
    'timestamp': int(time.time()),
        ...
        if response.status_code == 200:
        	...
        	print(g+f' ➜ Done Add 200 coin✅ | {coin} BY: @Y_1oo1')
        	tlg = f"https://api.telegram.org/bot{bot}/sendMessage?chat_id={acc}&text=•  ➜ Done Add 200 coin✅ | {coin} BY: @Y_1oo1"
        	tlg_params = {"parse_mode": "HTML"}
        	i = requests.post(tlg, params=tlg_params)
        else:
        	print(b+f' ➜ ERROR ADD COIN')
        	tlg = f"https://api.telegram.org/bot{bot}/sendMessage?chat_id={acc}&text=•  ➜ ERROR ADD COIN | {coin} BY:@Y_1oo1"
        	tlg_params = {"parse_mode": "HTML"}
        	i = requests.post(tlg, params=tlg_params)
        time.sleep(25)
